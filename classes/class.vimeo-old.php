...

{
    ...
    public function __construct() {
		...
		$this->site_prefix = VIMEO_PREFIX;

		$this->vimeo_user_id = '13474915';

        $this->_consumer_key = '1e704b013413985786022f661eb2357224ed349e';

        $this->_consumer_secret = '024a19ac45a696090c0ee9841bc31a375a80a986';

		$this->setToken( 'ac1062ff95b316362d1aaac0399bdd77', '05410348c55a5efeb5e622814be72d33a1d094f7' );
	...
	public function album_url( $album_id ) {
		...
		return 'http://player.vimeo.com/hubnut/album/' . $album_id . '?color=eabf14&amp;background=000000&amp;slideshow=0&amp;video_title=1&amp;video_byline=0';

	}

	

	public function get_album_previews() {

		$response = $this->call('vimeo.albums.getAll', array('user_id' => $this->vimeo_user_id, 'sort' => 'newest'));
		...
		foreach( $albums as $album ) {
			...
			if( !$this->site_prefix || substr( $album->title, 0, strlen( $this->site_prefix ) ) == $this->site_prefix ) {
				...
				$video = $this->call('vimeo.albums.getVideos', array('album_id' => $album->id, 'summary_response' => true));

				$arr[$a]['thumbnail'] = $video->videos->video[0]->thumbnails->thumbnail[1]->_content;
	...
	public function get_album( $album_id ) {
		...
		$response = $this->call('vimeo.albums.getAll', array( 'user_id' => $this->vimeo_user_id ));
		...
		foreach( $albums as $album ) {
			...
			if( $album->id == $album_id ) {
				...
				$arr['title'] = $this->site_prefix ? substr( $album->title, strlen( $this->site_prefix ) ) : $album->title;

				$arr['description'] = $album->description;

			}

		}

		$response = $this->call('vimeo.albums.getVideos', array('album_id' => $album_id, 'full_response' => true));

		$arr['lessons'] = $response->videos->total;

		$arr['runtime'] = 0;

		$arr['videos'] = $response->videos->video;

		foreach( $arr['videos'] as $key => $video ) {

			$arr['runtime'] += $video->duration;

			if( $video->upload_date < $arr['release_date'] || $key == 0 ) $arr['release_date'] = $video->upload_date;

			if( $video->modified_date > $arr['updated'] || $key == 0 ) $arr['updated'] = $video->modified_date;
	...
	public function get_video_previews($album_list = false) {

		$arr = array();

		$albums = $album_list ? $album_list: $this->build_album_array();

		foreach( $albums as $album_id => $album_detail ) {

			$arr[$album_id] = $this->get_album($album_id);
	...
	public function get_video( $video_id ) {
		...
		$response = $this->call('vimeo.videos.getInfo', array( 'video_id' => $video_id ) );
	...
	public function build_album_array() {
		...
		foreach( $response->albums->album as $a ) {

			if( !$this->site_prefix || strstr( $a->title, $this->site_prefix ) ) {

				$albums[$a->id] = (array) $a;

				if( $this->site_prefix ) $albums[$a->id]['title'] = substr( $albums[$a->id]['title'], strlen( $this->site_prefix ) );
	...
	public function sort_by_upload( $videos ) {
		...
		foreach( $videos as $video ) {
			...
			$key = $video->upload_date.str_pad($distinct, 4, '0', STR_PAD_LEFT);
	...
	public function update_database( $album_array = false ) {

		ob_end_flush(); ob_flush(); flush();

		mysql_query("TRUNCATE " . SEARCH_ALBUM_TABLE);

		mysql_query("TRUNCATE " . SEARCH_VIDEO_TABLE);
		...
		do{
			...
			$response = $this->call('vimeo.albums.getAll', array( 'user_id' => $this->vimeo_user_id, 'page' => $page ));

			foreach( $response->albums->album as $a ) {

				//echo str_pad( $a->id.'-'.$a->title.'<br>', 4096 );

				//ob_flush(); flush();

				if( ( !$this->site_prefix || strstr( $a->title, $this->site_prefix ) ) && ( !$album_array || in_array( $a->id, $album_array ) ) ) {

					$vid_response = $this->call('vimeo.albums.getVideos', array('album_id' => $a->id, 'full_response' => true));

					foreach( $vid_response->videos->video as $v ) {

						$sql = "INSERT INTO " . SEARCH_VIDEO_TABLE . " ( aID, vID, vTitle, vDescription, vDuration, vThumbnail ) VALUES ( " . $a->id . ", " . $v->id . ", '" . $v->title . "', '" . $v->description . "', '" . $v->duration . "', '" . $v->thumbnails->thumbnail[1]->_content . "' )";

						mysql_query( $sql );

						//echo str_pad( '-----'.$sql.'<br>', 4096 );

						echo str_pad( '---ADD VIDEO['.$v->id.']:'.$v->title.'<br>', 4096 );

						ob_flush(); flush();

					}

					$sql = "INSERT INTO " . SEARCH_ALBUM_TABLE . " ( aID, aTitle, aDescription, aVideos, aThumb ) VALUES ( " . $a->id . ", '" . $a->title . "', '" . $a->description . "', '" . $a->total_videos . "', '" . $vid_response->videos->video[0]->thumbnails->thumbnail[1]->_content . "' )";

					mysql_query( $sql );

					//echo str_pad( $sql.'<br>', 4096 );

					echo str_pad( 'ADD ALBUM['.$a->id.']:'.$a->title.'<br>', 4096 );
		...
		} while( ( $page * $response->albums->perpage ) <= $response->albums->total );

		echo str_pad( 'SYNCHRONIZATION COMPLETE<br>', 4096 );
