...
var ddimagepanner={
	...
	magnifyicons: ['img/icons/zoom_in.png','img/icons/zoom_out.png', 35,35], //set path to zoom in/out images, plus their dimensions
	...
	zoomfunct:function($, $img, s){
		...
		var $zoomimages=$('<img src="'+magnifyicons[0]+'" /><img src="'+magnifyicons[1]+'" />')
			.css({width:magnifyicons[2], height:magnifyicons[3], cursor:'pointer', zIndex:1000, position:'absolute',
						//top:s.wrappersize[1]-magnifyicons[3]-3, left:s.wrappersize[0]-magnifyicons[2]-3, opacity:0.7
						top:0, left:magnifyicons[2], opacity:0.7
			})
			.attr("title", "Zoom Out")
...
function size_pancontainer() {
	var img = $("#img_primary");
    var img_height = img.height();
	var img_width = img.width();
	var pan_width = img.parents(".pancontainer").width();
	var pan_height = pan_width / img_width * img_height;
	var msg = pan_width + '/' + img_width + '*' + img_height + '=' + pan_height;
	img.parents(".pancontainer").css('height',pan_height);
	img.width(pan_width);
}